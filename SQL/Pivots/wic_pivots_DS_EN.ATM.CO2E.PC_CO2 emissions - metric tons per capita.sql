...
--wszystkie roczniki odczytów dla wskaŸnika "CO2 emissions (metric tons per capita)"
--do tabeli przestawnej
select distinct  
	i.rocznik  
from indicators i
join country c2 on i.countrycode = c2.countrycode 
where i.indicatorcode='EN.ATM.CO2E.PC' and c2.currencyunit!=''
order by i.rocznik 
;

--skrajne roczniki odczytów dla wskaŸnika "CO2 emissions (metric tons per capita)"
select distinct 
...
--wszystkie odczyty dla wskaŸnika "CO2 emissions (metric tons per capita)"
...
from indicators i
join country c2 on i.countrycode = c2.countrycode 
where i.indicatorcode='EN.ATM.CO2E.PC' 
	and c2.currencyunit!=''
...
--najwy¿sze 10 odczytów dla wskaŸnika "CO2 emissions (metric tons per capita)" dla 2011 roku
...
,	(select i2.value 
	...
	where i2.indicatorcode='EN.ATM.CO2E.PC' 
	and c2.currencyunit!=''
	and i2.rocznik = '2011'
...
--najni¿sze odczyty dla wskaŸnika "CO2 emissions (metric tons per capita)" dla 2011 roku
--wszystkie kraje, czyli currency nie równa siê ''
select 
	c2.currencyunit
,	i.countryname 
...
--najni¿sze 10 odczytów dla wskaŸnika "CO2 emissions (metric tons per capita)" dla 2011 roku
--same nazwy krajów
select 
	i.countryname 
from indicators i
