...
--wszystkie roczniki odczytów dla wskaŸnika "Arable land (hectares per person)"
--do tabeli przestawnej
select distinct  
	i.rocznik  
from indicators i
join country c2 on i.countrycode = c2.countrycode 
where i.indicatorcode='AG.LND.ARBL.HA.PC' and c2.currencyunit!=''
order by i.rocznik 
;

--skrajne roczniki odczytów dla wskaŸnika "Arable land (hectares per person)"
select distinct 
...
--wszystkie odczyty dla wskaŸnika "Arable land (hectares per person)"
...
from indicators i
join country c2 on i.countrycode = c2.countrycode 
where i.indicatorcode='AG.LND.ARBL.HA.PC' 
	and c2.currencyunit!=''
...
--najwy¿sze 10 odczytów dla wskaŸnika "Arable land (hectares per person)" dla 2013 roku
...
,	(select i2.value 
	...
	where i2.indicatorcode='AG.LND.ARBL.HA.PC' 
	and c2.currencyunit!=''
	and i2.rocznik = '2013'
...
--najni¿sze odczyty dla wskaŸnika "Arable land (hectares per person)" dla 2013 roku
--wszystkie kraje, czyli currency nie równa siê ''
select 
	c2.currencyunit
,	i.countryname 
...
--najni¿sze 10 odczytów dla wskaŸnika "Arable land (hectares per person)" dla 2013 roku
--same nazwy krajów
select 
	i.countryname 
from indicators i
